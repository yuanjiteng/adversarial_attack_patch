        ...
        for i, det in enumerate(pred):  # per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    keep = np.in1d(predict_classes,attack_ids)
                    if np.sum(keep) > 0 :
                        # 识别到某种攻击类别
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='/data1/yjt/mydatasets/attack_datasets/images/', help='file/dir/URL/glob, 0 for webcam')
...
def parse_opt1():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default= '/data1/yjt/adversarial_attack/myattack_training_models/yolov5s-85-enhance.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default='/data1/yjt/adversarial_attack/myattack/res_imgs/', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640], help='inference size h,w')
    parser.add_argument('--conf-thres', type=float, default=0.75, help='confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.45, help='NMS IoU threshold')
