        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    keep = np.in1d(predict_classes,opt.attack_ids)
                    if np.sum(keep) > 0 :
                        # 识别到某种攻击类别
                        ...
                        cv2.imwrite(successed_predict_path, im0)
                    else:
                        # 没有识别到攻击类别
                        ...
                        cv2.imwrite(failed_predict_path, im0)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/data1/yjt/adversarial_attack/myattack_training_models/yolov7-85-enhance.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='/data1/yjt/mydatasets/attack_datasets/images/', help='source')  # file/folder, 0 for webcam
    # parser.add_argument('--source', type=str, default='/data/yjt/mydatasets/images/val/', help='source')
...
def parse_opt1():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default='/data1/yjt/adversarial_attack/myattack_training_models/yolov7-85-enhance.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='/data1/yjt/adversarial_attack/myattack/res_imgs/', help='source')  # file/folder, 0 for webcam
    # parser.add_argument('--source', type=str, default='/data/yjt/mydatasets/images/val/', help='source')
    parser.add_argument('--img-size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.5, help='object confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.45, help='IOU threshold for NMS')
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            for opt.weights in ['yolov7.pt']:
                successed_predict, failed_predict=detect(opt)
                strip_optimizer(opt.weights)
        else:
            successed_predict, failed_predict=detect(opt)
    
    with torch.no_grad():
        if opt1.update:  # update all models (to fix SourceChangeWarning)
            for opt.weights in ['yolov7.pt']:
                successed_predict1, failed_predict1=detect(opt1)
                strip_optimizer(opt1.weights)
        else:
            successed_predict1, failed_predict1=detect(opt1)
