...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, with_projection=True,attack_id=[4,7,80],pad_value=0):
            ...
            if(with_projection):
                ...
                adv_batch = adv_patch.expand(lab_batch.size(0), lab_batch.size(1), -1, -1, -1)  ##  torch.Size([4, 5, 3, 300, 300])
                ...
                # lab_batch 解码+选择有效部分
                lab_batch = lab_batch.view(b * f, 13)  # [8*14 13]
                ...
                dst = torch.narrow(lab_batch, 1, 5, 8)
...
class DataEnhancer(nn.Module):
    ...
    """
    def __init__(self,maskRoot,imgsize=640,maskNumPerCategory=30):
        ...
        rainMaskFiles =  os.listdir(rainMaskDir)
        fogMaskFiles =  os.listdir(fogMaskDir)
        Mask=[]
        transform = transforms.ToTensor()
        resize = transforms.Resize((self.imgsize, self.imgsize))
        for i,rainMaskFile in enumerate(rainMaskFiles):
            if i >= maskNumPerCategory:
                break
            mask = transform(Image.open(os.path.join(rainMaskDir,rainMaskFile)).convert('RGB')).unsqueeze(0)
            Mask.append(mask)
        for i,fogMaskFile in enumerate(fogMaskFiles):
            if i >= maskNumPerCategory:
                break
            mask = transform(Image.open(os.path.join(fogMaskDir,fogMaskFile)).convert('RGB')).unsqueeze(0)
            Mask.append(mask)
        Mask = torch.cat(Mask)
        Mask = resize(Mask)
    ...
    def forward(self,img_batch,beta=0.7,posibility=[0.1,0.5]):
        """
        功能：为每张img单独添加一个随机增强效果(雨、雾)
        输入：
            img_batch：贴上patch的图片组
            beta：控制mask权重,范围[0-1]由弱到强
        ...
        # 0 表示不加
        category_[category<=posibility[0]] = 0
        # 1 表示加雨
        category_[category>posibility[0]] = 1
        # 2 表示加雾
        category_[category>posibility[1]] = 2
        # 例如：category_ = 2 indexInOneCategory=0.51*30=15.3 indexInMask=45
        # 表示从总共61张mask中选中下标为45的mask(也即30张雨的mask中下标为15的mask)
        indexInMask = ((category_-1)*self.maskNumPerCategory+indexInOneCategory).to(int)
        indexInMask = torch.clamp(indexInMask,0,mask_num)
        selectedMask = beta*torch.index_select(self.Mask , 0, indexInMask).cuda()
        # 矩阵操作比for循环快
