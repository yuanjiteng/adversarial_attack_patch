...
class Backbone(nn.Module):
    def __init__(self, pretrain_path=None):
        ...
        self.feature_extractor = nn.Sequential(*list(net.children())[:7])#左闭右开
...
class SSD300(nn.Module):
    def __init__(self, backbone=None, num_classes=86):
        ...
        self._build_additional_features(self.feature_extractor.out_channels)#构建backbone之后的框架
        ...
        for nd, oc in zip(self.num_defaults, self.feature_extractor.out_channels):
            ...
            location_extractors.append(nn.Conv2d(oc, nd * 4, kernel_size=3, padding=1))#box   nd是box个数，4是回归参数
    ...
    def _build_additional_features(self, input_size):
        ...
        """
        additional_blocks = []
        # input_size = [1024, 512, 512, 256, 256, 256] for resnet50   input_size[:-1][1024, 512, 512, 256, 256,   input_size[1:] 512, 512, 256, 256, 256]
    ...
    def forward(self, image, targets=None):
        x = self.feature_extractor(image)#con4_x的输出  4 1024 38 38
...
class Loss(nn.Module):
    ...
    def _location_vec(self, loc):
        ...
        :return:
        """
        gxy = self.scale_xy * (loc[:, :2, :] - self.dboxes[:, :2, :]) / self.dboxes[:, 2:, :]  # Nx2x8732   这里是中心点和wh表示不是左上右下
        gwh = self.scale_wh * (loc[:, 2:, :] / self.dboxes[:, 2:, :]).log()  # Nx2x8732
        return torch.cat((gxy, gwh), dim=1).contiguous()

    def forward(self, ploc, plabel, gloc, glabel):#ploc预测的回归参数，预测的标签，
        # type: (Tensor, Tensor, Tensor, Tensor) -> Tensor
        """
            ploc, plabel: Nx4x8732, Nxlabel_numx8732
                predicted location and labels

            gloc, glabel: Nx4x8732, Nx8732   gloc  glabel在预处理阶段已经将正负样本划分，，正样本继续gt，负样本0，对应的box为anchor
                ground truth location and labels
        """
        # 获取正样本的mask  Tensor: [N, 8732]
        mask = torch.gt(glabel, 0)  # (gt: >)  在预处理阶段负样本设置成了0
        # mask1 = torch.nonzero(glabel)
        # 计算一个batch中的每张图片的正样本个数 Tensor: [N]
        pos_num = mask.sum(dim=1)#计算每张图片正样本的数量

        # 计算gt的location回归参数 Tensor: [N, 4, 8732]
        vec_gd = self._location_vec(gloc)#gt和预测的回归参数

        # sum on four coordinates, and mask
        # 计算定位损失(只有正样本)
        loc_loss = self.location_loss(ploc, vec_gd).sum(dim=1)  # Tensor: [N, 8732]
        loc_loss = (mask.float() * loc_loss).sum(dim=1)  # Tenosr: [N]去除掉负样本
        ...
        # 获取负样本
        con_neg = con.clone()
        con_neg[mask] = 0.0#得到负样本的值
        # 按照confidence_loss降序排列 con_idx(Tensor: [N, 8732])
        _, con_idx = con_neg.sort(dim=1, descending=True)#con_idx索引值
        _, con_rank = con_idx.sort(dim=1)  #升序# 这个步骤比较巧妙
