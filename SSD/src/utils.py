...
class Encoder(object):
    ...
    def encode(self, bboxes_in, labels_in, criteria=0.5):
        ...
        # [8732,]
        best_dbox_ious, best_dbox_idx = ious.max(dim=0)  # 寻找每个default box匹配到的最大IoU     每列最大值
        # [nboxes,]
        best_bbox_ious, best_bbox_idx = ious.max(dim=1)  # 寻找每个GT匹配到的最大IoU   每行最大值
...
class DefaultBoxes(object):
    def __init__(self, fig_size, feat_size, steps, scales, aspect_ratios, scale_xy=0.1, scale_wh=0.2):
        ...
        fk = fig_size / np.array(steps)     # 计算每层特征层的fk近似等于 self.feat_size = feat_size  # 每个预测层的feature map尺寸
        # [[2], [2, 3], [2, 3], [2, 3], [2], [2]]
        ...
        for idx, sfeat in enumerate(self.feat_size):
            sk1 = scales[idx] / fig_size  # scale转为相对值[0-1]    这里的比例是和原图比
