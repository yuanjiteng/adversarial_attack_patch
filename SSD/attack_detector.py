...
class MySSD():
    ...
    def attack(self, input_imgs, attack_id=[0], total_cls=85, object_thres=0.1, clear_imgs=None, compare_imgs=None,
        ...
        prof_max_scores = []
        any_max_scores = []
        data_transform = transforms_.Compose([transforms_.Resize(img_size),
                                              transforms_.Normalize(mean=[0.485, 0.456, 0.406],
                                                                    std=[0.229, 0.224, 0.225])
                                             ])
        #

        input_imgs_resize = data_transform(input_imgs).to(self.device)


        # input_imgs_resize1 = F.interpolate(input_imgs, size=img_size).to(self.device)#ssd输入大小规定300*300
        self.model.eval()

        predictions = self.model(input_imgs_resize)[0]
        classness = predictions[:, :, 5:]
        attack_classness = classness[:, :, attack_id]  #[B,x,3]
        confs, _ = torch.max(attack_classness, dim=1)  # [B,3]
