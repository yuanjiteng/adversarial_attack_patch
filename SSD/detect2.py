...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        predictions = model(im.to(device))[1][0]
        ...
        idxs = np.greater(predict_scores, conf_thres)
...
# weights_path = r"F:/Public/models_ubuntu/SSD2/model_10.pth"
def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default= "", help='model path(s)')
    parser.add_argument('--source', type=str, default=r'F:\Public\multiPatch\images', help='file/dir/URL/glob, 0 for webcam')
...
def parse_opt1():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default= "", help='model path(s)')
    parser.add_argument('--source', type=str, default=r'F:\XWF\project\version4\adversarial_attack_patch\patch_applier2\exp80\images_patch', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640], help='inference size h,w')
    parser.add_argument('--conf-thres', type=float, default=0.75, help='confidence threshold')
    parser.add_argument('--iou-thres', type=float, default=0.45, help='NMS IoU threshold')
    parser.add_argument('--device', default='2', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
