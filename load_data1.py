...
class PatchTransformer(nn.Module):
    ...
    def get_warpRMY(self,label,img_size, w, h):  # Points [torch.Tensor]
        ...
        org = org.unsqueeze(0)
        dst = dst.unsqueeze(0)
        if label[5] == 57600 or (dst > img_size).any():
    ...
    def resize_rotate(self,adv_batch,lab_batch,img_size,

                      with_black_trans,do_rotate,scale_rate,rand_loc,batch_size,by_rectangle=False):
        if (with_black_trans):
            adv_batch = torch.clamp(adv_batch, 0.0, 0.99999)
        else:
            adv_batch = torch.clamp(adv_batch, 0.000001, 0.99999)
        ...
        # Where the label class_id is 1 we don't want a patch (padding) --> fill mask with zero's
        cls_ids = torch.narrow(lab_batch, 2, 0, 1)  # torch.Size([4, 5, 1])
        cls_mask = cls_ids.expand(-1, -1, 3)  # torch.Size([8, 14, 3])
        cls_mask = cls_mask.unsqueeze(-1)  # torch.Size([8, 14, 3, 1])
        cls_mask = cls_mask.expand(-1, -1, -1, adv_batch.size(3))  # torch.Size([8, 14, 3, 300])
        cls_mask = cls_mask.unsqueeze(-1)  # torch.Size([8, 14, 3, 300, 1])
        cls_mask = cls_mask.expand(-1, -1, -1, -1, adv_batch.size(4))  # torch.Size([4,5,3,50,50])
        msk_batch = torch.cuda.FloatTensor(cls_mask.size()).fill_(90) - cls_mask  # torch.Size([8, 14, 3, 300, 300])
        msk_batch[msk_batch != 0] = 1
        # Pad patch and mask to image dimensionszai
        # Determine size of padding
        current_patch_size = adv_batch.size(-1)
        pad = (img_size - msk_batch.size(-1)) / 2  # (416-300) / 2 = 58
        mypad = nn.ConstantPad2d((int(pad), int(pad), int(pad), int(pad)), 0)
        # print("adv_batch size : "+str(adv_batch.size()))
        adv_batch = mypad(adv_batch)  # adv_batch size : torch.Size([8, 14, 3, 416, 416])
        ...
        # Theta = rotation,rescale matrix
        theta = torch.cuda.FloatTensor(anglesize, 2, 3).fill_(0)  # torch.Size([112, 2, 3])
        # scale取倒数 因为F.affine_grid 是反向采样
        theta[:, 0, 0] = (cos / scale)
        theta[:, 0, 1] = sin / scale
        theta[:, 0, 2] = (tx * cos / scale + ty * sin / scale)
        theta[:, 1, 0] = -sin / scale
    ...
    def forward(self, adv_patch, lab_batch, img_size, patch_mask=[], by_rectangle=False, do_rotate=True, rand_loc=True, with_black_trans=False, scale_rate=0.2, with_crease=False, with_projection=False, with_rectOccluding=False, enable_empty_patch=False, enable_no_random=False, enable_blurred=True,
        ...
        # adv_batch_masked, adv_batch_masked0, adv_batch_masked1, adv_batch_masked3, adv_batch_masked4, adv_batch_t, msk_batch_t = resize_rotate(adv_batch)
        # adv_batch_masked, msk_batch = self.resize_rotate(adv_batch,lab_batch,img_size,with_black_trans,do_rotate,scale_rate,rand_loc,batch_size,by_rectangle)  # adv_batch torch.Size([8, 7, 3, 150, 150])   adv_batch_masked torch.Size([8, 7, 3, 416, 416])
        ...
        if(with_projection):
            ...
            lab_batch_scaled = lab_batch_scaled.view(b_*f_, c_)
            adv_batch_projected = []
            for i,(label,adv_patch1) in enumerate(zip(lab_batch_scaled,adv_batch)):
                mat = self.get_warpRMY(label,img_size, w = adv_patch_size, h = adv_patch_size)
            ...
            adv_batch = adv_batch.view(b, f, c, img_size, img_size)
            pad = (img_size - adv_batch.size(-1)) / 2  # (416-300) / 2 = 58
            mypad = nn.ConstantPad2d((int(pad), int(pad), int(pad), int(pad)), 0)
            adv_batch = mypad(adv_batch)  # adv_batch size : torch.Size([8, 14, 3, 416, 416])
            ...
            #do rotate and scale
            adv_batch_masked, msk_batch = self.resize_rotate(adv_batch,lab_batch,img_size,with_black_trans,do_rotate,scale_rate,rand_loc,batch_size,by_rectangle)
        # return adv_batch_masked, adv_batch_masked0, adv_batch_masked1, adv_batch_masked3, adv_batch_masked4, adv_batch_t, msk_batch_t, adv_patch_set
        return adv_batch_masked, adv_patch_set, msk_batch
...
class InriaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # label 个数如果小于 max_lab 那么扩充到 max_lab
        label = self.pad_lab(label)
        return image, label
    
    # label 和 image 进行填充缩放
